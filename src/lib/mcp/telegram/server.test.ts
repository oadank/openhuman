...
describe("TelegramMCPServer", () => {
  ...
  beforeEach(async () => {
    ...
    const { SocketIOMCPTransportImpl } = await import("../transport");
    vi.mocked(SocketIOMCPTransportImpl).mockImplementation(function (this: any) {
    ...
    vi.mocked(getChatsApi.getChats).mockResolvedValue({
      data: [
        { id: "1", title: "Chat One", type: "private", unreadCount: 0, isPinned: false },
        { id: "2", title: "Group Chat", type: "group", unreadCount: 5, isPinned: true },
      ],
      fromCache: true,
    });
    ...
    vi.mocked(sendMsgApi.sendMessage).mockResolvedValue({
      data: { id: "msg1", message: "Hello", chatId: "1", date: 1234567890 } as any,
      fromCache: false,
    });

    const getUserApi = await import("./api/getCurrentUser");
    vi.mocked(getUserApi.getCurrentUser).mockResolvedValue({
      data: { id: "1", firstName: "Alice", lastName: "Smith", username: "alice", isBot: false } as any,
      fromCache: true,
    });
    ...
    vi.mocked(getMsgsApi.getMessages).mockResolvedValue({
      data: [
        { id: "m1", message: "First message", chatId: "1", date: 1234567890, fromId: "100" } as any,
        { id: "m2", message: "Second message", chatId: "1", date: 1234567891, fromId: "200" } as any,
      ],
      fromCache: true,
    });
    ...
    const toolCallEntry = onCalls.find(
      (call: any[]) => call[0] === "toolCall",
    );
