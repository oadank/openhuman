...
import type { ToolCaptureConfig } from "../tool-compress";
...
describe("compressObservation", () => {
  ...
  it("should compress web_search", () => {
    const result = compressObservation("web_search", {
...
describe("compressMessagesForSummary", () => {
  const makeMessages = (): Message[] => [
    {
      role: "user",
      content: [{ type: "text", text: "Search for my preferences" }],
    },
    {
      role: "assistant",
      content: [
      ...
      content: [
        {
          type: "tool_result",
          toolUseId: "tu_2",
          content: "Successfully wrote to memory.md",
        },
      ],
    },
  ];
  ...
  it("should skip tool_result for skipped tools", () => {
    ...
    const searchResult = compressed.find(
      (m) =>
        m.content.some(
          (c) =>
            c.type === "tool_result" &&
  ...
  it("should preserve text blocks unchanged", () => {
    ...
    expect(userMsg!.content[0]).toEqual({
      type: "text",
      text: "Search for my preferences",
    });
  });
  ...
  it("should respect custom config", () => {
    ...
    const hasWriteToolUse = compressed.some((m) =>
      m.content.some(
        (c) => c.type === "tool_use" && c.name === "memory_write",
      ),
    );
