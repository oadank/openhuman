...
export function detectArchitecture(): Architecture {
  ...
  if (/mac/i.test(platform) && /cpu os \d+_\d+_0 like mac os x/i.test(userAgent)) {
    ...
    if (match) {
      ...
      if (major >= 11) {
        // Rely on user agent check above for ARM
      }
    }
  }
...
/**
 * Parse GitHub release assets and map them to platforms (legacy)
 */
...
export function getDownloadLink(platform: Platform, releaseLinks?: PlatformDownloadLinks): string {
  ...
  const links: Record<Platform, string> = {
    windows: 'https://example.com/download/openhuman-windows.exe',
    macos: 'https://example.com/download/openhuman-macos.dmg',
    linux: 'https://example.com/download/openhuman-linux.AppImage',
    android: 'https://example.com/download/openhuman-android.apk',
